...
export const startInterview = async (sessionId: string): Promise<StartInterviewResponse> => {
  const response = await api.post<StartInterviewResponse>(`/start-interview?session_id=${sessionId}`);
  return response.data;
};

export const getNextQuestion = async (sessionId: string): Promise<QuestionResponse> => {
  const response = await api.get<QuestionResponse>(`/next-question?session_id=${sessionId}`);
  return response.data;
};
...
export const getFinalResult = async (sessionId: string): Promise<FinalReport> => {
  const response = await api.get<FinalReport>(`/final-result?session_id=${sessionId}`);
  return response.data;
};
